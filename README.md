...
| A | Bytes & the B+Tree | `0x03`–`0x08` | next |
...
`internal/page` fixes the unit at **4096 bytes** — the machine's virtual-memory
page size, and a whole number of 512-byte disk sectors. Postgres uses 8 KB,
...
- `Page.Append` writes items with no separator, so `"scute-dbhello"` cannot be
  split back apart → `0x03`
